...
```{r}
# multipleChoiceResponses$LearningPlatformSelect
...
ProveKnowledge_select <- MultipleChoiceOptions_func("ProveKnowledgeSelect") 

hchart(ProveKnowledge_select, type="column", hcaes(x = value, y = N, group = value),
                                   options3d = list(enabled = TRUE, beta = 1, alpha = 1))%>%
  # title and subtitle
...
# hchart(net)
# hchart(net, layout = layout_in_circle)
...
multipleChoiceResponses %>%
  # making duplicates of the original currency column as *m.*
  .[, m.CompensationAmount:= CompensationAmount] %>%
  .[, m.CompensationCurrency:= CompensationCurrency]
```
...
multipleChoiceResponses %>%
  ...
  # adapting ikleiman's treatment of salary, where if the salary is between 6 and 36 times of the median, it is * 12
...
multipleChoiceResponses%>%
  ...
  .[Country=="India"& CompensationCurrency=="INR",.(CompensationAmount,CompensationCurrency)] %>%
  # There exist individuals who are taking foreign currency, perhaps they are doing remote work or working overseas? In any case,
  .[, if(.N >10) .SD, by = list(CompensationCurrency)]%>%
  # filtering out those with absurd compensation amount, there are two with 0 or hundreds, selecting only those greater than 10000 (perhaps they are taking internships?)  
...
multipleChoiceResponses %>%
  .[!is.na(m.CompensationAmount), 
    list(N=.N,
         NA.P = sum(is.na(m.CompensationCurrency))
    ), by= Country] %>%
  .[order(-N)]

# multipleChoiceResponses$ExchangeRate
...
# plot of hcboxplot
hcboxplot(x = t.WTF$m.CompensationUSD, var = t.WTF$Country) %>%
  hc_xAxis(labels = list(rotation=0)) %>%
  hc_yAxis(title = list(text="USD")) %>%
  hc_title(text = "Boxplot of Pay in USD per country") 
...
t.cut<-multipleChoiceResponses[!is.na(Country),] %>%
  .[,.(NCount=.N, 
       ...
       WorkingExpMedian= median(m.Experience)
   ), 
   by=c("CountryCode")]
...
#plotting for highcharter map
hcmap(mapData = worldgeojson, 
      download_map_data=F, data = t.cut, value = "WorkingP",
      joinBy = c("iso-a2", "CountryCode"), name = "Demography",
      dataLabels = list(enabled = F, format = '{point.name}'), borderWidth = 1,
      tooltip = list(valueDecimals = 2, valuePrefix = "$", valueSuffix = " USD")) 
