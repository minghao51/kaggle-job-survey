...
* The majority ( of not all the plots) are donoe with highcharter.
...
* multipleChoiceResponse ( For survey questions that a list of selections are given ( sometimes mulitple selections are accepted)
...
I also used a Country-Currency-Code list from github, as some of the analysis required country to currency mapping( to determine foreign currencies). Also, the Country Name in the given .csv are mapped to iso2 for plotting in hmap ( highchart's map).
...
* MultipleChoice_func : To aggregate and summarise column in multipleChoiceResponses.csv that follows certain pattern. For example, multipleChoiceResponses(Country) would output a list of country with aggregated counts. Also the input will be melted into long format while aggregatation to various factor as in the case multipleChoiceResponses(Jobfactor) in which case there is multiple column of JobFactorCommute, JobFactorCompanyFunding etc 

* MultipleChoiceOptions_func : For survey questions that accept multiple selection, since these columns require strsplit treatment before aggregation and summarising as the contents are stored in comma seperated list. Ie, MultipleChoiceOptions_func(PastJobTitlesSelect)
...
```
**People 's Republic of China - China** and  **Republic of China - Taiwan** is available for selective as seperate entries. Both of the names are coverted into shorter, one word form. Also, noted that there are seperate entries for selection for Hong Kong.

Given that this is the results of a survey done on Kaggle, there would be some natural bias towards kaggle and english medium. There may exist other data science platforms that are more popular in Other countries, especially if that country's medium of communication is another languages(China, Japan, Germany).
...
```
Plenty seems are intending to turn towards data science, well. This is probably the reason why most of us are here anyway. To learn , practice and prove ourselves.
...
* OriginCountry : (USD, EUR, AUD) which is more indicative of currency, the column name is somewat misleading. 
...
The conversion rates dt need to be joined with the country-currencies input, which can be used to indentify the original currencies of the respective countries, allowing us to identify number of individual earning foreign currencies.
...
I think this previous selections has  for proven knowledge has limited the respondents to choose only amount the listed choices (ie, bias in the sense that most people are unlikely to select "Other" option and fill in projects, instead prefer to choose amount the available options). Among the choice I think that are missing from the list are projects, open-sourced packages contributions, blogs and publication.
...
Well regression is still the most impactful algorithm, I guess it pays to be simple and easily interpretable. All other popular algorithm such as random forest, neural network, logistic regression are also mentioned.

# Mixed Visualisation

Most of the kernals that I had seen looks to explore the surveys results with barcharts. While it easily shows which are the most popular factor, the bar chart may appear misleading if there are less people submitting response for a particular section, or that the respondents are divided, dividing into two extremes. Some respondents may just not be familier with a particular option.

I attributed each availabe factor to a linear scale (not that it has to be linear), such a that for a 3 factor list, it would be considered as 0, 1, 2 instead, the higher number are giving to a more favourable condition ( the higher the better). 

Also, since the list of questions are identical for a particular extraction of datasets ( ie Job Factor, I will just adapt a random one of the exact question from the scheme to depict the question clearly).
...
Interestingly, while the bar chart shows that company/funding is also rather important, there exist a certain segments of individuals who think it is not as important (thereby lowering the score). 

Overall, I think the spine plot better depicts Learners' preferance for opportunities in Learning, Office, Salary, Languages and Management. Value concepts and abtracts notions such as diversity and leader reputation are second to their consideration. This seems better comfort to the my normal expectations.
...
I'm rather surprise to see "Select" here. Especially when it scored rather low on the WorkMethodsFrequency.
...
The color should be indicate of communities in the network graph, acquired via **cluster_walktrap**. Sadly, while highchart's plot is interactive, I couldn't get the arrow to be shown in the network graph ( to shown where have the individuals been moving).

The Weight options for the width of the lines are manually set to reduce the visibility of less job title changes. Alternative, one could implement a cutoff/threhold value as to when to draw a line connecting the nodes.

The size of the individual nodes are indicative of the population of respective current job title.

Overall, it seems Data Scientist are the core of the network plots, with most of its network coming from data analyst, Software Developer/Software Engineer, and Researcher.


# Differences of Percentage in of The Jobtitle of current and past.

If we plot the percentage difference between job title of current and past ( though, not all positions are equal, some roles may be appear more frequency simply due to it being a high competative filed).
...
This shows that the job titles of Data scientist and Scientist/Researcher have been becoming popular overtime, ie, while the percentage of those who once hold title of programmer and data analyst has decreased. I wonder if this means that Kaggle survey respondents have been slowly moving into their field of choice (Data Science) after parting from their past jobs.  
...
Hence, the compensations listed here is probably more appropriate to those employed and compensated locally instead of remotely, as we will see in next section, country like Isreal, Ukraine, India bosted a significant number of individual earning foregin currencies
...
USD is the most popular foregin currency by far for those who earn foreign currencies. Surprisingly, Ukraine, showed up right after India to as one of the more popular country for its individuals to receive compensation in USD. I wonder if it is because some of the individuals holding top jobs in the country demand salary in USD or that this is a popular practice at certain regions
...
For those with 1-2 years into the field, it seems that the compensation does somewat correlate with the level of education. Though, unfortunately, it seems the effect of education  ( degree level and above) is less pronounce after 6 to 10 years. Perhaps it doesn't pays to be highly educated for a long ran?

Though, please take this with a pinch of salt as DS is a new field, eventually, certain positions may simply require minimun Master's degree to even be considered ( as per other fields, the fault of specialisation we have these days).
...
While title doesn't seems to be worthly important to learners ( in Mixed Visuallsation - Job Factor section), the respective compensation one receive after years in the industry can be quite different.
...
* Ncount - Indicative of number of individulas 
...
highchart()%>%
  hc_add_series_map(worldgeojson, df = t.cut, value = "LookingForJobP", joinBy = "iso2",
                    dataLabels = list(enabled = TRUE,
...
t.cut<-multipleChoiceResponses[!is.na(Country),] %>%
  .[,.(NCount=.N, 
       ...
       DS_P=signif(mean(CurrentJobTitleSelect=="Data Scientist", na.rm=T),2),
       Research_P=signif(mean(CurrentJobTitleSelect=="Researcher" | CurrentJobTitleSelect=="Scientist/Researcher ", na.rm=T),1)
       ), 
  by=c("CountryCode", "Country")]
...
hc_tooltip(useHTML = TRUE,
             headerFormat = "<table>",
             pointFormat = paste("<tr><th colspan=\"1\"><td>{point.Country}</td></th></tr>",
                                 "<tr><th>LocalPay.Med<td>{point.CompensationLocal_Median}</td></th></tr>",
                              "<tr><th>LocalPay.Q3<td>{point.CompensationLocal_Q3}</td></th></tr>",
                                 "<tr><th>LocalPay.Q1</th><td>{point.CompensationLocal_Q1}</td></tr>",
                                 "<tr><th>USDPay.Med</th><td>{point.CompensationUSD_Median}</td></tr>",
                                 "<tr><th>DS.P</th><td>{point.DS_P}</td></tr>",
                               "<tr><th>Research.P</th><td>{point.Research_P}</td></tr>"),
             footerFormat = "</table>")
